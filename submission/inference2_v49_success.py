...
class Policy:
    def __init__(self, model_dir: str = None):
        print("[MORES] v49 - Ultra Survival Strategy for F2")
        
        # ========== 阶段1：极端保守（步数 1-40）==========
        self.kp_ip_stage1 = 0.70
        self.kp_r_stage1 = 1.90
        self.kp_z_stage1 = 3.20
        self.action_low_stage1 = -400
        self.action_high_stage1 = 400
        ...
        print("[MORES] v49 Ultra Survival Strategy for F2 ready")
        print(f"  Stage1 (1-40): ±{self.action_high_stage1}V, gains={self.kp_ip_stage1}/{self.kp_r_stage1}/{self.kp_z_stage1}")
        print(f"  Stage2 (41-120): ±{self.action_high_stage2}V, gains={self.kp_ip_stage2}/{self.kp_r_stage2}/{self.kp_z_stage2}")
        print(f"  Stage3 (121+): ±{self.action_high_stage3}V, gains={self.kp_ip_stage3}/{self.kp_r_stage3}/{self.kp_z_stage3}")
    ...
    def act(self, observation: Dict) -> np.ndarray:
        ...
        if self.step_count in [1, 41, 121]:
            stage = "Stage1" if self.step_count <= 40 else ("Stage2" if self.step_count <= 120 else "Stage3")
