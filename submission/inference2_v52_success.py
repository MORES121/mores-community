...
class Policy:
    def __init__(self, model_dir: str = None):
        print("[MORES] v52 - Extreme Survival (Action Range ±150V Early)")
        
        # ========== 阶段1：极端保守（步数 1-70）==========
        self.kp_ip_stage1 = 0.85
        self.kp_r_stage1 = 2.20
        self.kp_z_stage1 = 3.80
        self.action_low_stage1 = -150
        self.action_high_stage1 = 150
        ...
        print("[MORES] v52 ready")
        print(f"  Stage1 (1-70): ±{self.action_high_stage1}V, gains={self.kp_ip_stage1}/{self.kp_r_stage1}/{self.kp_z_stage1}")
        print(f"  Stage2 (71-180): ±{self.action_high_stage2}V, gains={self.kp_ip_stage2}/{self.kp_r_stage2}/{self.kp_z_stage2}")
        print(f"  Stage3 (181+): ±{self.action_high_stage3}V, gains={self.kp_ip_stage3}/{self.kp_r_stage3}/{self.kp_z_stage3}")
    ...
    def act(self, observation: Dict) -> np.ndarray:
        ...
        if self.step_count in [1, 71, 181]:
            stage = "Stage1" if self.step_count <= 70 else ("Stage2" if self.step_count <= 180 else "Stage3")
            print(f"[F1] Step {self.step_count}: {stage}")
