...
class Policy:
    def __init__(self, model_dir: str = None):
        print("[MORES] v51 - Extreme Survival for F2")
        
        # ========== 阶段1：极端保守（步数 1-60）==========
        self.kp_ip_stage1 = 0.80
        self.kp_r_stage1 = 2.10
        self.kp_z_stage1 = 3.60
        self.action_low_stage1 = -200
        self.action_high_stage1 = 200
        ...
        print("[MORES] v51 ready for F2")
        print(f"  Stage1 (1-60): ±{self.action_high_stage1}V, gains={self.kp_ip_stage1}/{self.kp_r_stage1}/{self.kp_z_stage1}")
        print(f"  Stage2 (61-160): ±{self.action_high_stage2}V, gains={self.kp_ip_stage2}/{self.kp_r_stage2}/{self.kp_z_stage2}")
        print(f"  Stage3 (161+): ±{self.action_high_stage3}V, gains={self.kp_ip_stage3}/{self.kp_r_stage3}/{self.kp_z_stage3}")
    ...
    def act(self, observation: Dict) -> np.ndarray:
        ...
        elif self.step_count <= 160:
            action = self.table_stage2.get(key, self.default_action)
        else:
            action = self.table_stage3.get(key, self.default_action)
        
        if self.step_count in [1, 61, 161]:
            stage = "Stage1" if self.step_count <= 60 else ("Stage2" if self.step_count <= 160 else "Stage3")
