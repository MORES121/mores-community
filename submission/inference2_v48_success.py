...
class Policy:
    def __init__(self, model_dir: str = None):
        print("[MORES] v48 - Extreme Survival Strategy for F2")
        
        # 阶段1：极端保守（步数 1-30）
        self.kp_ip_stage1 = 0.65
        self.kp_r_stage1 = 1.80
        self.kp_z_stage1 = 3.00
        self.action_low_stage1 = -500
        self.action_high_stage1 = 500
        ...
        print("[MORES] v48 Extreme Survival Strategy for F2 ready")
        print(f"  Stage1 (1-30): ±{self.action_high_stage1}V, gains={self.kp_ip_stage1}/{self.kp_r_stage1}/{self.kp_z_stage1}")
        print(f"  Stage2 (31-100): ±{self.action_high_stage2}V, gains={self.kp_ip_stage2}/{self.kp_r_stage2}/{self.kp_z_stage2}")
        print(f"  Stage3 (101+): ±{self.action_high_stage3}V, gains={self.kp_ip_stage3}/{self.kp_r_stage3}/{self.kp_z_stage3}")
    ...
    def act(self, observation: Dict) -> np.ndarray:
        ...
        if self.step_count in [1, 31, 101]:
            stage = "Stage1" if self.step_count <= 30 else ("Stage2" if self.step_count <= 100 else "Stage3")
